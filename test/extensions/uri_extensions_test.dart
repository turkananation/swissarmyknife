...
void main() {
  group('UriKnife', () {
    group('addQueryParams', () {
      ...
      test('should handle iterable values (lists) for duplicate query keys', () {
        final uri = Uri.parse('https://example.com/posts');
        final updated = uri.addQueryParams({'tags': ['dart', 'flutter']});
    ...
    group('removeQueryParams', () {
      test('should remove specified query params', () {
        final uri = Uri.parse('https://example.com?id=123&token=abc&role=admin');
