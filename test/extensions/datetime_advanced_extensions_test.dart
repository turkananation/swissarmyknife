  ...
  group('DateTimeAdvancedKnife', () {
    group('truncateTo', () {
      test('should truncate to specific intervals correctly', () {
        ...
        expect(
          date.truncateTo(DateTimeInterval.month),
          equals(DateTime(2026, 6)),
        );
        expect(
          date.truncateTo(DateTimeInterval.day),
          equals(DateTime(2026, 6, 26)),
        );
        expect(
          date.truncateTo(DateTimeInterval.hour),
          equals(DateTime(2026, 6, 26, 15)),
        );
        expect(
          date.truncateTo(DateTimeInterval.minute),
          equals(DateTime(2026, 6, 26, 15, 30)),
        );
        expect(
          date.truncateTo(DateTimeInterval.second),
          equals(DateTime(2026, 6, 26, 15, 30, 45)),
        );
      });
  ...
  group('DurationKnife', () {
    group('toFormattedString', () {
      ...
      test('should handle sub-second values', () {
        expect(
          const Duration(milliseconds: 500).toFormattedString(),
          equals('500ms'),
        );
        expect(
          const Duration(microseconds: 5).toFormattedString(),
          equals('5us'),
        );
    ...
    group('toHumanReadable', () {
      test('should format duration as human readable', () {
        ...
        expect(
          dur.toHumanReadable(),
          equals('2 days, 4 hours, 12 minutes, 30 seconds'),
        );
      });
